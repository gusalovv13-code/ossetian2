...
test("публичные товары ограничены активными и видимыми", () => {
  assert.match(serverSource, /COALESCE\(status, 'active'\) = \$1/);
...
test("пользовательское удаление является мягким", () => {
  assert.match(
    serverSource,
    /app\.delete\("\/api\/products\/:id"[\s\S]*?UPDATE products[\s\S]*?status = 'deleted'[\s\S]*?hidden = TRUE/
  );
  assert.doesNotMatch(
    serverSource,
    /app\.delete\("\/api\/products\/:id"[\s\S]*?DELETE FROM products/
  );
});
